...
$(function () {
  ...
  var currentAction = {
    ...
    persistentSnackbar: $.mSnackbar.add({
      text: 'Welcome! Here are some controls<br><b>Undo/redo</b>: Ctrl + Z/Y<br><b>Delete</b>: Delete/Backspace',
      lifespan: Infinity
    })
  };
      ...
      switch (user.mode) {
        case modes.rectangle:
          ...
          currentAction.fabricObject.set({
            width: -(Math.abs(currentAction.startCoord[0]) - Math.abs(e.offsetX)),
            height: -(Math.abs(currentAction.startCoord[1]) - Math.abs(e.offsetY))
          });
  ...
  function refreshPolygon(polygon) {
    ...
    if (canvas.getObjects()[canvas.getObjects().length - 1] &&
      canvas.getObjects()[canvas.getObjects().length - 1].get('type') == 'polygon') {
